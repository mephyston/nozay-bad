...
1. Remplacement de la détection de transaction imbriquée par `catch (err) { if (err.message.includes('begin')) ... }` dans `libs/domains/accounting/commands/reconcile-bank-statement-line/handler.ts` par une vérification explicite et orientée objet avec `db instanceof SQLiteTransaction` de `drizzle-orm/sqlite-core`.
2. Les méthodes `reconcileBankStatementLine` et `reconcileBulkTransactions` vérifient si l'objet `db` fourni en entrée est déjà une transaction active. Si oui, elles exécutent l'action directement sans tenter de recréer une transaction imbriquée. Sinon, elles créent la transaction englobante.
...
```
 ...
 ✓  features-accounting-ui  ../commands/reconcile-bank-statement-line/ui/BankStatementReconciliation.test.ts (12 tests) 526ms
