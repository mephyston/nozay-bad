# Refactor Checks Router Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract checks and check-deposits handlers from `routes.ts` into a separate sub-router file `routes/checks.ts`, mount them back on `routes.ts`, and ensure all tests continue to pass.

**Architecture:** Create two Hono sub-routers (`checksRouter` and `checkDepositsRouter`) in a new file `routes/checks.ts`. Export them and mount them on `/checks` and `/check-deposits` in `routes.ts`, deleting the original handlers from `routes.ts`.

**Tech Stack:** TypeScript, Hono, Drizzle ORM.

## Global Constraints
- Do not modify database schemas.
- Ensure all types match `Bindings` from `routes.ts`.
- No circular dependencies or lint errors.

---

### Task 1: Create checks.ts router file

**Files:**
- Create: `libs/features/accounting/api/src/routes/checks.ts`

**Interfaces:**
- Consumes: `Bindings` from `../routes`, helper `cleanName` from `../helpers`
- Produces: `checksRouter` and `checkDepositsRouter`

- [ ] **Step 1: Write `routes/checks.ts` containing the check and check deposit handlers**
...
### Task 2: Modify routes.ts to mount the new routers

**Files:**
- Modify: `libs/features/accounting/api/src/routes.ts`

**Interfaces:**
- Consumes: `checksRouter` and `checkDepositsRouter` from `./routes/checks`
- Produces: Updated `accountingRouter`

- [ ] **Step 1: Import sub-routers and mount them**
  Import the routers and mount them on `/checks` and `/check-deposits`. Remove old imports and endpoints.
  Ensure `checksTable` and `checkDepositsTable` are removed from imports if no longer used in `routes.ts`.
...
**Interfaces:**
- Consumes: Test scripts
- Produces: Correct test execution
