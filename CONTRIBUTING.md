...
4. **Rien qui nomme le club** : nom, sigle, adresse, mentions légales, IBAN, préfixes, images des documents, fonctionnalités utilisées vivent dans `club_settings` / `club_features` (domaine `libs/domains/club`), réglés depuis **Réglages → Configuration du club**. Le code les lit par `getClubSettings(db)` (API et domaines), `locals.club` (les trois applications) ou `@nba/club-ui` (composants). Ni variable de déploiement ni constante : `libs/club-identity.test.ts` refuse toute source qui nomme le club. Les seules exceptions sont les migrations (qui sèment le club existant), `scripts/build-env.mjs` (les domaines déployés) et les tests. Une fonctionnalité **inachevée** reste, elle, derrière un drapeau de livraison `*_ENABLED` (§8.1) : le club décide de ce qu'il utilise, le dépôt de ce qui est livré.
5. **Adresses des applications** : elles ne s'écrivent que dans `scripts/build-env.mjs` (table `URLS` par environnement), inlinées au build par `PUBLIC_WEBSITE_URL`, `PUBLIC_STOREFRONT_URL`, `PUBLIC_SITE_URL`. Un build hors développement sans ces variables échoue en CI (`CI=true`) et retombe sur `localhost` en local.
