...
import { analyzeBankTransactionsRoute } from './route';
import { setupMockDb } from '@metacult/shared-db/test-utils';
import { analyzeBankTransactions } from './handler';

vi.mock('./handler', () => ({
  analyzeBankTransactions: vi.fn(),
}));

describe('analyzeBankTransactions Route', () => {
  it('should return 400 when season is missing', async () => {
    const { mockD1 } = await setupMockDb();
    const aiMock = {};
    const res = await analyzeBankTransactionsRoute.request(
  ...
  it('should return 200 on valid query parameters', async () => {
    ...
    vi.mocked(analyzeBankTransactions).mockResolvedValue(mockData as any);
    ...
    expect(analyzeBankTransactions).toHaveBeenCalledWith(
      expect.anything(),
      expect.anything(),
