...
describe('CreateTransaction Route', () => {
  ...
  it('should return 200 on valid body', async () => {
    const { mockD1 } = await setupMockDb();
    vi.mocked(createTransaction).mockResolvedValue({ id: 1 });
  ...
  it('should return 400 when handler throws an error', async () => {
    const { mockD1 } = await setupMockDb();
    vi.mocked(createTransaction).mockRejectedValue(new Error('Saison clôturée'));
