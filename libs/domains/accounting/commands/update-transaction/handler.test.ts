...
import * as membersDataAccess from '@metacult/features-members-data-access';
...
describe('updateTransaction', () => {
  it('should update a transaction successfully', async () => {
    ...
    const result = await updateTransaction(mockDb, mockId, mockDto);
  ...
  it('should throw an error if original season is closed', async () => {
    ...
    await expect(updateTransaction(mockDb, mockId, mockDto)).rejects.toThrowError(/La saison d'origine est clôturée/);
  });
});
