<script lang="ts">
  ...
  import type { ReconciliationState, BankTransaction } from './reconciliation.svelte';

  let { state }: { state: ReconciliationState } = $props();

  function renderAiSuggestions(bt: BankTransaction) {
      ...
      <Tabs.Root bind:value={state.activeRightTab} class="w-full">
        ...
        <Tabs.Content value="manual">
          <CreateTransactionFromBankLine
            ...
            sortedMembers={state.filteredMembers}
            memberDisplayVal={state.memberDisplayVal}
            categoryDisplayVal={state.categoryDisplayVal}
            bind:category={state.category}
            bind:paymentMethod={state.paymentMethod}
            bind:selectedMemberId={state.selectedMemberId}
            bind:amountToLink={state.amountToLink}
            ...
            bind:memberHighlightedIndex={state.memberHighlightedIndex}
            bind:categoryHighlightedIndex={state.categoryHighlightedIndex}
            remainingAmount={state.remainingAmount}
            seasons={state.seasons}
            selectMember={state.selectMember}
            handleMemberKeyDown={state.handleMemberKeyDown}
            selectCategory={state.selectCategory}
            handleCategoryKeyDown={state.handleCategoryKeyDown}
