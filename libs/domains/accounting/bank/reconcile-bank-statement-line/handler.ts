    ...
    if (body.transactions && Array.isArray(body.transactions)) {
      for (const txItem of body.transactions) {
        ...
        const valeurs = {
          seasonId,
          type: txItem.type,
          accountId: await resolveAccountId(db, txItem.accountId, { active: true }),
          category: normalizeCategory(txItem.category),
          amount: Math.round(txItem.amount),
          date: txItem.date,
          paymentMethod: (await resolvePaymentMethod(db, txItem.paymentMethod, { active: true })).id,
      ...
      const valeurs = {
        seasonId,
        type: tx.type,
        accountId: await resolveAccountId(db, tx.accountId, { active: true }),
        category: normalizeCategory(tx.category),
        amount: Math.round(tx.amount),
        date: tx.date,
        paymentMethod: (await resolvePaymentMethod(db, tx.paymentMethod, { active: true })).id,
