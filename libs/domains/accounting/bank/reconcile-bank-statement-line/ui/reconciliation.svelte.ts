...
export function createReconciliationState(initialProps: ReconciliationStateProps) {
  ...
  async function handleBulkReconcile() {
    ...
    try {
      const requests = [];
      for (const id of ids) {
        const bt = bankStatementLines.find(t => t.id === id);
  ...
  safeEffect(() => {
    if (!selectedTx && bankStatementLines.length > 0 && typeof sessionStorage !== 'undefined') {
      ...
      if (savedIdStr) {
        ...
        const found = bankStatementLines.find(t => t.id === savedId && t.status === activeTab);
        if (found) {
          selectedTx = found;
        }
      }
  ...
  return {
    get bankStatementLines() { return bankStatementLines; },
    set bankStatementLines(v) { bankStatementLines = v; },
