...
import { updateBankStatementLineStatus, updateBankStatementLineStatuses } from './handler';
import { updateBankTransactionStatusBulkSchema, updateBankTransactionStatusParamSchema } from './validator';
...
const handleBulkStatus = async (c: any) => {
  if (!c.env || !c.env.DB) {
    return c.json({ success: false, error: 'Database binding DB is missing' }, 500);
  }
  ...
  const db = createDb(c.env.DB);
  try {
    const result = await updateBankStatementLineStatuses(db, body);
...
const bulkStatusValidator = tbValidator('json', updateBankTransactionStatusBulkSchema, (result, c) => {
  if (!result.success) {
    return c.json({ success: false, error: 'Validation failed: ' + [...result.errors].map(e => `${(e as any).path || (e as any).instancePath?.replace(/^\//, '') || 'field'}: ${e.message}`).join(', ') }, 400);
  }
});

/* Les routes de lot se déclarent avant celles à paramètre : `/status-bulk` serait sinon capté
   par `/:id/ignore` et rejeté comme identifiant invalide. */
updateBankStatementLineStatusRoute.post('/bank-statement-lines/status-bulk', bulkStatusValidator, handleBulkStatus);
updateBankStatementLineStatusRoute.post('/bank-transactions/status-bulk', bulkStatusValidator, handleBulkStatus);
