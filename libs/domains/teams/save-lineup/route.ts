...
import { saveLineupSchema } from './validator';

export type Bindings = { DB: D1Database };

export const saveLineupRoute = new Hono<{ Bindings: Bindings }>();

saveLineupRoute.put(
  '/:id/days/:number/lineup',
  tbValidator('json', saveLineupSchema, (result, c) => {
  ...
  async (c) => {
    ...
    return c.json({
      success: true,
      data: await saveLineup(createDb(c.env.DB), { teamId, dayNumber, ...c.req.valid('json') })
    });
  }
);
