...
if [ -z "$health" ] || ! echo "$health" | grep -E -q '"ok":\s*true'; then
...
success=$(echo "$dummy" | (jq -r 'if .success == null then "missing" else .success | tostring end' 2>/dev/null || echo "missing"))
errors=$(echo "$dummy" | (jq -r 'if .["error-codes"] == null then 0 else .["error-codes"] | length end' 2>/dev/null || echo "0"))
