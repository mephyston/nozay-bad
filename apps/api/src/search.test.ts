...
import { searchRouter, matches } from './search';
...
const search = async (env: any, q: string) => {
  const res = await app.request(`http://localhost/search?q=${encodeURIComponent(q)}&seasonCode=25-26`, undefined, env);
...
describe('GET /search', () => {
  it('ne cherche rien sous deux lettres', async () => {
    ...
    expect(data).toEqual({ members: [], posts: [], events: [], teams: [], products: [] });
  });
  ...
  it('trouve un adhérent, un article publié, un événement et un produit, chacun avec son chemin', async () => {
    ...
    expect(posts.map((p: any) => p.href)).toEqual(['/actualites#tournoi-de-noel']);
